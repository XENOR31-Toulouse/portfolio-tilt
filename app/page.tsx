      ...
      <div className="relative">
        ...
        <section className="mx-auto max-w-7xl px-6 pt-20 md:pt-28">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div className="max-w-3xl">
              <p className="text-sm font-medium text-violet-300/80 uppercase tracking-wider">Disponible – {profile.location}</p>
              <h1 className="mt-2 text-4xl md:text-6xl font-bold tracking-tight">
                {profile.name} <span className="block text-2xl md:text-3xl font-semibold text-violet-300">{profile.title}</span>
              </h1>
              ...
              <div className="mt-6 flex flex-wrap items-center gap-3">
                <a href={`mailto:${profile.contact.email}`} className="px-4 py-2 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 transition">Email</a>
                <a href={`tel:${profile.contact.phone.replace(/\s+/g, '')}`} className="px-4 py-2 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 transition">Téléphone</a>
                <a href={profile.links.linkedin} target="_blank" rel="noreferrer" className="px-4 py-2 rounded-xl border border-violet-500/30 bg-violet-500/10 hover:bg-violet-500/20 transition">LinkedIn</a>
                <a href="/cv/Axel_Ibanez_CV.pdf" target="_blank" className="px-4 py-2 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 transition">Télécharger le CV (PDF)</a>
              </div>
        ...
        <section className="mx-auto max-w-7xl px-6 pt-12 pb-12">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {profile.experience.slice(0,3).map((exp) => (
              <div key={exp.role+exp.company} className="rounded-2xl border border-white/10 bg-white/5 p-5">
        ...
        <section id="projects" className="mx-auto max-w-7xl px-6 pb-24">
          <div className="flex items-center justify-between">
            ...
            <Link href="/projects" className="text-sm text-violet-300 underline underline-offset-4">Tout voir</Link>
          </div>
